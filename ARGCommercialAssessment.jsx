...
// Set this to your deployed API's base URL once hosted (Railway/Render/etc).
// Left as localhost for continued local testing against the engine.
const ARG_API_BASE_URL = "http://127.0.0.1:8000";
...
function mapApiResponseToDashboardResult(apiResponse) {
  ...
  const score = Math.round(proc.overall_score ?? 0);
  ...
  // to show the number and let the client read it directly.
  const confidencePct = ar.confidence ?? 0;
  ...
  const constraints = ar.dominant_constraints || [];

  const vulnerabilities = constraints
    .filter((c) => (c.effective_pressure ?? 0) < 0)
    .slice(0, 3)
    .map((c) => c.name || c.mechanism);

  const strengths = constraints
    .filter((c) => (c.effective_pressure ?? 0) >= 0)
    .slice(0, 3)
    .map((c) => c.name || c.mechanism);

  // intervention_paths carries the real reasoning-chain output, now read
  // from its confirmed location at processing.intervention_paths (not
  // nested under adaptive_recoverability). Mapped down to plain strings
  // here as a stopgap — the upgrade path renders structural_observation /
  // adaptive_objective / pathway_categories / advisory_scope directly
  // instead of collapsing them into one line.
  const recommendations = (proc.intervention_paths || []).map(
  ...
  return {
    ...
    recommendations,
    domainScores,
    // Full raw response retained for the dashboard upgrade — nothing is
    // thrown away, it's just not rendered until ExecutiveDashboard is updated.
    _raw: apiResponse,
  };
}
      ...
      >
        ...
        <ul style={{ margin: 0, paddingLeft: 20 }}>
          {result.recommendations.map((r) => (
            <li key={r} style={{ color: COLORS.silverDim, fontSize: 14, marginBottom: 8, lineHeight: 1.5 }}>
              {r}
            </li>
          ))}
        </ul>
...
export default function ARGCommercialAssessment() {
  ...
  const handleRunAssessment = async () => {
    ...
    try {
      const apiResponse = await fetchAdaptiveRecoverabilityAssessment(template, responses);
