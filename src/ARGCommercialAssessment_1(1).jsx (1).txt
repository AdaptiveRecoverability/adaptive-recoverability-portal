...
function mapApiResponseToDashboardResult(apiResponse) {
  ...
  // "Strengths" is currently derived from nonnegative-pressure records
  // within dominant_constraints, NOT from an independent backend strengths
  // ranking — dominant_constraints appears intended primarily to surface
  // constraints, so a positive record's presence here isn't necessarily a
  // formally ranked "top strength." Revisit if/when the API adds an
  // explicit strengths field; until then this is the best available signal,
  // clearly labeled as such in the UI copy (see empty-state text below).
  const vulnerabilities = dominantConstraints
    .filter((c) => (c.effective_pressure ?? 0) < 0)
    .slice(0, 3)
    .map(getConstraintLabel);

  const strengths = dominantConstraints
    .filter((c) => (c.effective_pressure ?? 0) >= 0)
    .slice(0, 3)
    .map(getConstraintLabel);
  ...
  // Kept for any code still reading the flattened form during transition —
  // new rendering should use interventionPaths directly.
  const recommendations = interventionPaths.map(
    (p) => p.structural_observation || p.reasoning || "Review this area with your governance team."
  );

  const adaptiveState = proc.adaptive_state || null;
          ...
          {result.strengths.length > 0 ? (
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              {result.strengths.map((s) => (
                <li key={s} style={{ color: COLORS.silverDim, fontSize: 13.5, marginBottom: 8, lineHeight: 1.5 }}>
                  {s}
                </li>
              ))}
            </ul>
          ...
          {result.vulnerabilities.length > 0 ? (
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              {result.vulnerabilities.map((v) => (
                <li key={v} style={{ color: COLORS.silverDim, fontSize: 13.5, marginBottom: 8, lineHeight: 1.5 }}>
                  {v}
                </li>
              ))}
            </ul>
