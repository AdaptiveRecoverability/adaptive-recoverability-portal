...
// for the plan to render dominant_constraints/intervention_paths directly.
function mapApiResponseToDashboardResult(apiResponse) {
  ...
  //   dominant constraints → processing.adaptive_recoverability.dominant_constraints
  ...
  // Backend returns dominant_constraints in its own priority order.
  // Filtering (not sorting) preserves that order — we don't re-rank by
  // effective_pressure or any other metric, since that would override
  // whatever ordering logic the engine applies.
  // NOTE: an earlier sanity check on this same engine found dominant_constraints
  // and intervention_paths using two different orderings for the same items
  // (dominant_constraints wasn't strictly sorted by magnitude, intervention_paths
  // was). That should be re-verified against a fresh response before this
  // ordering is treated as fully authoritative — filtering here is the correct
  // frontend behavior either way, but "correct order" still depends on the
  // backend actually returning one consistent order.
  const dominantConstraints = Array.isArray(ar.dominant_constraints) ? ar.dominant_constraints : [];

  // ---- dashboard_summary integration ----
  // UNVERIFIED CLAIM, HANDLED DEFENSIVELY: a report from the API-side model
  // describes a new processing.dashboard_summary field (top_strengths,
  // top_vulnerabilities, priority_recommendations, strength_details,
  // vulnerability_details, recommendation_details) as now authoritative.
  // Unlike every other contract change in this build, that report did not
  // include an actual raw API response — only prose/reconstructed JSON
  // snippets — and this field has not appeared in any Swagger response
  // pasted into this project so far. Rather than block on that (or blindly
  // trust it), this reads dashboard_summary ONLY if it's genuinely present
  // with real arrays, and falls back to the existing dominant_constraints
  // derivation otherwise — so this is safe whether or not the field turns
  // out to be real. Confirm against an actual pasted Swagger response
  // before treating dashboard_summary as load-bearing.
  const dashboardSummary = proc.dashboard_summary && typeof proc.dashboard_summary === "object"
    ? proc.dashboard_summary
    : null;

  const hasBackendStrengths = dashboardSummary && Array.isArray(dashboardSummary.top_strengths);
  const hasBackendVulnerabilities = dashboardSummary && Array.isArray(dashboardSummary.top_vulnerabilities);
  const hasBackendRecommendations = dashboardSummary && Array.isArray(dashboardSummary.priority_recommendations);

  // "Strengths" derived from nonnegative-pressure records within
  // dominant_constraints is the legacy fallback — used only when the
  // backend hasn't supplied an explicit ranked list. NOT necessarily a
  // formally ranked "top strength," just the best available signal
  // without dashboard_summary.
  const vulnerabilities = hasBackendVulnerabilities
    ? dashboardSummary.top_vulnerabilities
    : dominantConstraints
        .filter((c) => (c.effective_pressure ?? 0) < 0)
        .slice(0, 3)
        .map(getConstraintLabel);

  const strengths = hasBackendStrengths
    ? dashboardSummary.top_strengths
    : dominantConstraints
        .filter((c) => (c.effective_pressure ?? 0) >= 0)
        .slice(0, 3)
        .map(getConstraintLabel);
  ...
  const strengthDetails =
    dashboardSummary && Array.isArray(dashboardSummary.strength_details) ? dashboardSummary.strength_details : [];
  const vulnerabilityDetails =
    dashboardSummary && Array.isArray(dashboardSummary.vulnerability_details)
      ? dashboardSummary.vulnerability_details
      : [];
  const recommendationDetails =
    dashboardSummary && Array.isArray(dashboardSummary.recommendation_details)
  ...
  // Prefer backend-ranked recommendation text when dashboard_summary is
  // present; otherwise fall back to the flattened intervention_paths text.
  const recommendations = hasBackendRecommendations
    ? dashboardSummary.priority_recommendations
    : interventionPaths.map(
        (p) => p.structural_observation || p.reasoning || "Review this area with your governance team."
      );
  ...
  return {
    ...
    // True only when dashboard_summary was genuinely present with real
    // arrays — lets the UI know whether it's showing backend-ranked
    // commercial labels or the legacy dominant_constraints-derived fallback.
    usingDashboardSummary: Boolean(hasBackendStrengths || hasBackendVulnerabilities || hasBackendRecommendations),
    dominantConstraints,
    interventionPaths,
    domainScores,
    adaptiveState,
...
/* ----------------------------------------------------------------------
   ...
   scenario selection re-runs POST /calculate with a real `environment`
   value, and the backend's own result is shown as-is — no frontend
   scoring, classification, or reweighting.

   PORTAL_ENVIRONMENTS is deliberately narrower than the full set of valid
   API environment keys. The frozen API accepts five ("normal",
   "governance_shift", "liquidity_shock", "cyber_attack", "crisis"), but
   only two are exposed in this first mock-client portal release — the
   other three are a later, deliberate portal-scope expansion, not an API
   limitation.
   ---------------------------------------------------------------------- */

const PORTAL_ENVIRONMENTS = [
  { id: "normal", label: "Normal Operating Environment" },
  { id: "liquidity_shock", label: "Liquidity Shock" },
];

function StressTestView({ result, template, responses, onBack }) {
  const [environment, setEnvironment] = useState("normal");
  const [scenarioResult, setScenarioResult] = useState(
    environment === "normal" ? result : null
  );
  ...
  const runScenario = useCallback(
    async (env) => {
      // "normal" with no evidence changes is just the original baseline result —
      // no need to re-call the API for the environment this assessment already ran under,
      // unless the baseline itself wasn't run under "normal".
      setIsLoading(true);
      setError(null);
      try {
        const apiResponse = await fetchAdaptiveRecoverabilityAssessment(template, responses, env);
        setScenarioResult(mapApiResponseToDashboardResult(apiResponse));
      } catch (err) {
        setError(err.message || "The assessment engine could not be reached.");
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const handleSelectEnvironment = (env) => {
    ...
    // "normal" is the environment the baseline assessment was already run
    // under (explicitly, since the main submit flow now sends
    // environment: "normal" rather than relying on a backend default) — so
    // re-selecting it restores that original result instead of re-calling
    // the API for a scenario that's already been calculated.
    if (env === "normal") {
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "48px 24px 80px" }}>
      ...
      >
        Each scenario below re-runs the assessment engine under a different
        operating environment. Results are calculated live, not simulated.
      </p>
          ...
          {PORTAL_ENVIRONMENTS.map((env) => (
            <button
              ...
              style={{
                ...
                padding: "9px 16px",
                borderRadius: 999,
                border: `1px solid ${environment === env.id ? COLORS.gold : COLORS.border}`,
                background: environment === env.id ? "rgba(212,175,55,0.1)" : "transparent",
                color: environment === env.id ? COLORS.gold : COLORS.silverDim,
                fontFamily: "'Inter', sans-serif",
                fontSize: 13,
                fontWeight: 500,
            ...
            >
              <span
                style={{
                  ...
                  height: 8,
                  borderRadius: "50%",
                  background: environment === env.id ? COLORS.gold : "transparent",
                  border: `1px solid ${environment === env.id ? COLORS.gold : COLORS.silverDim}`,
                }}
              />
              {env.label}
            </button>
      ...
      {isLoading && (
        <div style={{ textAlign: "center", color: COLORS.silverDim, fontSize: 13.5, marginBottom: 32 }}>
          Recalculating under this environment…
        </div>
      )}

      {error && (
        <div style={{ textAlign: "center", color: COLORS.red, fontSize: 13.5, marginBottom: 32 }}>
          {error}
        </div>
      )}

      {!isLoading && !error && scenarioResult && (
        <>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}>
            <ScoreDial score={scenarioResult.score} color={scenarioResult.statusColor} />
          </div>
          <div
            style={{
              textAlign: "center",
              ...
              fontSize: 15,
              fontWeight: 600,
              color: scenarioResult.statusColor,
              marginBottom: 6,
            }}
          >
          ...
          >
            Baseline: {result.score} ({result.status}) → {scenarioResult.score} under{" "}
            {PORTAL_ENVIRONMENTS.find((e) => e.id === environment)?.label}
          </div>
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "48px 24px 80px" }}>
      ...
      >
        ...
        {result.interventionPaths.length === 0 ? (
          <p style={{ color: COLORS.silverDim, fontSize: 14, lineHeight: 1.5, margin: 0 }}>
            No intervention pathways were returned for this assessment.
          </p>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {result.interventionPaths.map((p, i) => {
              // intervention_paths objects use `mechanism_type`, not
              // `mechanism` (confirmed against Swagger — the two arrays use
              // different field names for the same concept). Normalize
              // before reusing getConstraintLabel, which expects `mechanism`.
              const title =
